...
class calls_record{
	...
	//calls_record(const vcf_line_cols& O): INFO_col(O.INFO_col) {}
...
void denovo_worker(std::istream& fin,const std::string& trio_ID_filename,std::ostream& fout,const std::string& X_choice,const double PP_thresh,const double ExAC_thresh){
	...
	//ofstream fout(outfilename1.c_str());
	string line;
	getline(fin,line);
	...
	//const double PP_thresh(0.005);
	//const double ExAC_thresh(0.008);
	...
	if(ELDO.LOC.chrom>=1 && ELDO.LOC.chrom<=upper_limit && ExAC_AF<ExAC_thresh && MDQ_rank>=17){
		...
		if(PP>PP_thresh){
			//den_writer(PP,EM_data_obj,O,ELDO,fout)<<"\n";
